...
#remove non ny state waters
st <- ne_ohi_rgns %>%
  filter(rgn_name %in% c("Connecticut", "Rhode Island", "Massachusetts-Virginian"))

a <- as(mab, 'Spatial') - as(st, 'Spatial')

#remove state boundaries
b <- a - as(states, 'Spatial')

# need to erase land on CT and RI.

land <- states %>%
  filter(NAME %in% c("Rhode Island", "Connecticut")) %>%
  st_buffer(0.05)

mab_out <- st_erase(st_as_sf(b), land)
...
mab_out <- st_crop(mab_out, xmin = -76, ymin = 39, xmax = -69, ymax = 41.45) %>%
  rename(rgn_name = longname) %>%
  mutate(rgn_id = 4) %>%
  select(-name)
...
ne_ohi_rgns_out <- ne_ohi_rgns %>%
  filter(rgn_name !="Mid-Atlantic Bight") %>%
  rbind(mab_out) %>%
  arrange(rgn_id) %>%
...
```{r}
st_write(ne_ohi_rgns_out, "shapefiles/ne_ohi_rgns.shp", delete_layer = T)
```
